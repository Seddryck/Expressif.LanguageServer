...
<sub>CMD:</sub>
```bash
type "data.json" | expressif.LanguageServer --stdin -t template.hbs -r json -o output.txt
```

<sub>PowerShell:</sub>
```powershell
Get-Content data.json | expressif.LanguageServer --stdin -t template.hbs -r json -o output.txt
```

<sub>Bash:</sub>
```bash
cat data.json | expressif.LanguageServer --stdin -t template.hbs -r json -o output.txt
```
...
<sub>PowerShell:</sub>
```powershell
expressif.LanguageServer --stdin -t template.hbs -r json
```
