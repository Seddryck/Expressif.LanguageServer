...
```bash
expressif.LanguageServer -t template.hbs -s data.txt -r json -o output.txt
```
...
```bash
expressif.LanguageServer -t template.txt -s data.json -X dat:Json;fm:FrontMatter -o output.txt
```
...
```powershell
expressif.LanguageServer -t template.txt -s data.json -e handlebars -o output.txt
```
...
```powershell
expressif.LanguageServer -t template.txt -s data.json -x txt:handlebars;liquid:fluid -o output.txt
```
