...
```bash
expressif.LanguageServer -t template.hbs -s data.tsv -X tsv:csv -P tsv@delimiter:Semicolumn;tsv@commentChar:#
```
...
The following parameters are accepted by Expressif.LanguageServer to define the behavior of a CSV parser:
...
To avoid conflicts with other parts of the command line, Expressif.LanguageServer supports the following synonyms for parameter values:
